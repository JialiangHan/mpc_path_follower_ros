...
{
    // looks good
    void MPC::setWeighting(const double &rho, const double &rhoN)
    {
        ...
        for (int i = 1; i < N_; ++i)
        {
            Qx_.coeffRef(i * number_of_state_ - 2, i * number_of_state_ - 2) = rho;
            Qx_.coeffRef(i * number_of_state_ - 1, i * number_of_state_ - 1) = 0;
        }
        Qx_.coeffRef(N_ * number_of_state_ - 4, N_ * number_of_state_ - 4) = rhoN;
        Qx_.coeffRef(N_ * number_of_state_ - 3, N_ * number_of_state_ - 3) = rhoN;
        Qx_.coeffRef(N_ * number_of_state_ - 2, N_ * number_of_state_ - 2) = rhoN * rho;
    }
    // looks good
    ...
    Eigen::SparseMatrix<double> MPC::setupqx(const VectorX &x0)
    {
        ...
        for (int i = 0; i < N_; ++i)
        {
            ...
            qx.coeffRef(i * number_of_state_ + 3, 0) = -Qx_.coeffRef(i * number_of_state_ + 3, i * number_of_state_ + 3) * v;
    ...
    // looks good
    void MPC::updateAdBdgd(const double &arc_length, double &x, double &y, double &last_phi)
    {
    ...
    // looks good
    Eigen::MatrixXd MPC::setupHessian()
    {
    ...
    // looks good
    Eigen::MatrixXd MPC::setupGradient(const VectorX &x0, const Eigen::SparseMatrix<double> &qx)
    {
    ...
    // looks good
    std::vector<Eigen::MatrixXd> MPC::setStateConstrain(const double &v_max)
    {
        std::vector<Eigen::MatrixXd> state_constrain_vec;
        Eigen::MatrixXd lx, Cx, ux;
    ...
    // looks good
    std::vector<Eigen::MatrixXd> MPC::setControlConstrain(const double &a_max, const double &steering_angle_max, const double &steering_angle_rate_max)
    {
        std::vector<Eigen::MatrixXd> control_constrain_vec;
        Eigen::MatrixXd lu, Cu, uu;
        // a delta constrains
        ...
        for (int i = 0; i < N_; ++i)
        {
            ...
            lu.coeffRef(i * 3 + 0, 0) = -a_max;
            uu.coeffRef(i * 3 + 0, 0) = a_max;
            // min steering angle<=steering angle<=max steering angle
            Cu.coeffRef(i * 3 + 1, i * number_of_control_ + 1) = 1;
            lu.coeffRef(i * 3 + 1, 0) = -steering_angle_max;
            uu.coeffRef(i * 3 + 1, 0) = steering_angle_max;
            ...
            if (i == 0)
            {
                // lu_.coeffRef(2, 0) = predictInput_.front()(1) - steering_angle_rate_max * dt_;
                // uu_.coeffRef(2, 0) = predictInput_.front()(1) + steering_angle_rate_max * dt_;
                lu_.coeffRef(2, 0) = -steering_angle_max;
                uu_.coeffRef(2, 0) = +steering_angle_max;
            }
            else
            {
                lu.coeffRef(i * 3 + 2, 0) = -steering_angle_rate_max * dt_;
                uu.coeffRef(i * 3 + 2, 0) = steering_angle_rate_max * dt_;
            }
        }
        state_constrain_vec.emplace_back(Cu);
        state_constrain_vec.emplace_back(lu);
        state_constrain_vec.emplace_back(uu);
        return control_constrain_vec;
    }
    // looks good
    std::vector<Eigen::MatrixXd> MPC::combineConstrain(const std::vector<Eigen::MatrixXd> &state_constrain, const std::vector<Eigen::MatrixXd> &control_constrain, const Eigen::SparseMatrix<double> &x0_sparse, const Eigen::SparseMatrix<double> &BB_sparse, const Eigen::SparseMatrix<double> &AA_sparse, const Eigen::SparseMatrix<double> &gg_sparse)
    {
        std::vector<Eigen::MatrixXd> result;
        Eigen::MatrixXd A, lower_limit, upper_limit;
        ...
        Eigen::VectorXd lower_limit_d = lower_limit.toDense();
        Eigen::VectorXd upper_limit_d = upper_limit.toDense();
        result.emplace_back(A);
        result.emplace_back(lower_limit_d);
        result.emplace_back(upper_limit_d);
        return result;
    }
    // looks good
    int MPC::solveMPC(const VectorX &x0_observe, const double &rho, const double &rhoN)
    {
        historyInput_.pop_front();
        historyInput_.push_back(predictInput_.front());

        setWeighting(rho, rhoN);
        ...
        AA_ = matrix_vec[1];
        gg_ = matrix_vec[2];
        Eigen::SparseMatrix<double> qx = setupqx();
        Eigen::SparseMatrix<double> BB_sparse = BB_.sparseView();
        Eigen::SparseMatrix<double> AA_sparse = AA_.sparseView();
        Eigen::SparseMatrix<double> gg_sparse = gg_.sparseView();

        Eigen::MatrixXd hessian_matrix = setupHessian();
        Eigen::MatrixXd gradient_matrix = setupGradient(x0, qx);

        std::vector<Eigen::MatrixXd> state_constrain_vec = setStateConstrain(v_max);
        std::vector<Eigen::MatrixXd> control_constrain_vec = setControlConstrain(a_max, steering_angle_max, steering_angle_rate_max);
        std::vector<Eigen::MatrixXd> constrain_vec = combineConstrain(state_constrain_vec, control_constrain_vec, x0_sparse, BB_sparse, AA_sparse, gg_sparse);

        int ret = solveQP(hessian_matrix, gradient_matrix, constrain_vec[0], constrain_vec[1], constrain_vec[2]);
    ...
    // looks good
    int MPC::solveQP(const Eigen::MatrixXd &hessian, const Eigen::MatrixXd &gradient, const Eigen::MatrixXd &linearMatrix, const Eigen::MatrixXd &lowerBound, const Eigen::MatrixXd &upperBound)
    {
    ...
    {
        double result;
        result = (p1.head(2) - p2.head(2)).norm;
        return result;
    }
    // looks good
